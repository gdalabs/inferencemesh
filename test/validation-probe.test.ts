...
describe('the reply budget of a live check', () => {
  ...
  test('is the same request in every place that checks a key or a model', () => {
    ...
    const req = validationRequest('vendor/m');
    assert.equal(req.model, 'vendor/m');
    assert.equal(req.max_tokens, VALIDATION_MAX_TOKENS);
  ...
  test('carries a prompt a cache has not been asked before', () => {
    // The budget fix must not quietly undo the anti-cache prompt: a provider
    // serving a cached 200 would make every one of these checks green.
    const a = validationRequest('vendor/m').messages[0]?.content;
    const b = validationRequest('vendor/m').messages[0]?.content;
    assert.notEqual(a, b, 'two checks must not send the same string');
  });
});
