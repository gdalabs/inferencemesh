...
무료 한도를 묶는 라우터는 새로운 발상이 아닙니다. [`docs/prior-art.md`](docs/prior-art.md)에서
아홉 개를 읽고, **이 프로젝트의 어떤 아이디어가 독창적이지 않은지**를 먼저 적어 두었습니다.
무료 한도 회계도, 민감도 기반 라우팅도 독창적이지 않습니다.
드물다고 말할 수 있는 것은 세 가지뿐입니다.

- **유료 모델로 넘어가지 않습니다.** 조사한 게이트웨이는 예외 없이 유료 폴백을 기능으로
  갖고 있습니다. 여기서는 무료 한도를 다 쓰면 오류입니다. 무료 한도에 의존하는 사람이
  청구서가 오는 것을 가장 알아채기 어려운 사람이기 때문입니다
- **저장한 키를 다시 읽어낼 방법이 없습니다.** `KeyStore`에 `get`이 없습니다.
  암호화해서 보관하는 것이 아니라 **꺼내는 통로 자체가 없습니다**. 반환하는 엔드포인트도,
  로그에 남기는 곳도 없습니다
- **레지스트리에 추측이 하나도 없습니다.** 검증 날짜가 없는 0이 아닌 가격은 검증에서 떨어지고,
  아무도 probe 하지 않은 능력은 선언하지 않습니다. 이런 실수는 오류를 내지 않습니다.
  조용히 라우팅 순서를 바꿀 뿐입니다. 그래서 검토가 아니라 기계로 강제합니다
...
| 클라이언트 | 가능 여부 | |
|---|---|---|
| [OpenCode](https://opencode.ai) | **가능** | 실제로 통과시켜 확인 |
| OpenAI 호환 클라이언트·SDK 전반 | **가능** | `/v1/chat/completions`가 계약의 전부 |
| [Codex CLI](https://github.com/openai/codex) | **불가** | `/v1/responses`가 필요 |
| [Claude Code](https://code.claude.com) | **불가** | Anthropic Messages API가 필요 |

설정 방법과 이 표의 근거가 된 실측은 [`docs/clients.md`](docs/clients.md)에 있습니다.
**불가** 두 줄은 문서를 읽고 판단한 것이 아니라 **직접 연결해 실패시켜** 확인했습니다.
두 도구 모두 미리 알려주지 않습니다.
